...
| 锻炼计划 | 计划可增删改，内置 4 套默认计划；两种排程模式：按星期绑定每周自动切换，或「练 N 休 M」滚动周期（训练日在所选计划间轮转，休息日显示恢复卡片，月历打休息标记，连续打卡跳过休息日不断签，手动选计划即临时加练） |
| 打卡 | 勾选即时保存、自动划掉变灰；同一天同一动作 Upsert 不重复 |
| 自定义项目 | 在计划里临时加动作，支持目标值 / 当前值 / 单位（如 俯卧撑 20 个） |
| 三餐记录 | 早 / 中 / 晚三列，每列默认推荐最常记录的 3 样（可直接勾选、改名，✕ 不再显示该推荐但历史记录保留，重新录入即恢复），跟随所选日期变化 |
...
| 数据安全 | 所有删除类操作均先确认并写明影响范围；SQLite 每次启动自动滚动备份（保留最近 7 份） |
| 账号体系 | 邮箱注册 / 登录，验证码走邮件，连续失败临时锁定；密码框带显隐小眼睛；登录后可设语言、主题、身体信息 |
...
SQLite 用户每次启动服务时会自动在 `data/backups/` 留一份当日滚动备份（保留最近 7 份），误删或 bug 丢数据时可回滚到前一天的状态；服务常驻不重启不会产生新备份。
